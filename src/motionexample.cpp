...
void swingToAngle(Vec3 initial_angle, Vec3 final_angle, double total_time,
                  double run_time, double cycle_time, string side,
                  RobotCmd &cmd, const RobotData &data) {
  Vec3 goal_angle;
  ...
  ///< The following two methods can be used to complete joint control. By
  ///< default, position control mode is used, and hybrid control mode is
  ///< supported

  ///< Joint pd control - position control method, namely, issuing kp, kd, goal_
  ///< angle，goal_ Vel, feedforward force tor=0, joint end completes joint pd
  ///< control closed-loop
  if (true) {
    ...
    ///< Joint pd control - force control method, i.e. kp, kd, goal_
    ///< angle，goal_ Set level to 0 The upper layer obtains real-time joint
    ///< data and compares it with the target, performs pd control, calculates
    ///< the expected torque of each joint, and sends it as a feedforward force
    ///< tor to the joint end, which directly executes the target torque
    ...
    cmd.joint_cmd[3 * leg_side].torque =
        60 * (goal_angle[0] -
              data.joint_data.joint_data[3 * leg_side].position) +
        0.7 * (goal_velocity[0] -
               data.joint_data.joint_data[3 * leg_side].velocity);
    cmd.joint_cmd[3 * leg_side + 1].torque =
        80 * (goal_angle[1] -
              data.joint_data.joint_data[3 * leg_side + 1].position) +
        0.7 * (goal_velocity[1] -
               data.joint_data.joint_data[3 * leg_side + 1].velocity);
    cmd.joint_cmd[3 * leg_side + 2].torque =
        80 * (goal_angle[2] -
              data.joint_data.joint_data[3 * leg_side + 2].position) +
        0.7 * (goal_velocity[2] -
               data.joint_data.joint_data[3 * leg_side + 2].velocity);
  }
}
...
/// @brief Specifically achieve swinging one leg of the robot to a specified
...
/// @param side Control which leg, FL is the left front leg, FR is the right
/// front leg, HL is the left and right leg, and HR is the right rear leg
/// @param cmd Issue control command
/// @param data Real-time status data of robot

/// @brief Interpolation to find the path point, i.e. the target angle for each
/// control cycle
/// @param init_position
/// @param init_velocity
/// @param goal_position
...
/// @param sub_goal_position_next2 Target angle for the next and next control
/// cycle
void cubicSpline(double init_position, double init_velocity,
                 double goal_position, double goal_velocity, double run_time,
                 double cycle_time, double total_time,
                 double *const sub_goal_position,
                 double *const sub_goal_position_next,
                 double *const sub_goal_position_next2) {
  double a, b, c, d;
  d = init_position;
  c = init_velocity;
  a = (goal_velocity * total_time - 2 * goal_position +
       init_velocity * total_time + 2 * init_position) /
      pow(total_time, 3);
  b = (3 * goal_position - goal_velocity * total_time -
       2 * init_velocity * total_time - 3 * init_position) /
      pow(total_time, 2);

  if (run_time > total_time)
    run_time = total_time;
  *sub_goal_position =
      a * pow(run_time, 3) + b * pow(run_time, 2) + c * run_time + d;
